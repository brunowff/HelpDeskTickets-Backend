...
import br.com.doubletelecom.help_desk_tickets.app.domain.dtos.PageTicketLogDto;
...
public class TicketLogController {
    ...
    @PreAuthorize("hasAuthority('SCOPE_API_BASIC') or hasAuthority('SCOPE_API_LOG')")
    public ResponseEntity<PageTicketLogDto> findAll(@RequestParam(defaultValue = "0") int page,
                                        @RequestParam(defaultValue = "10") int pageSize){
        
        try {
            var ticketLogs = ticketLogServices.findAll(page, pageSize);
            return ResponseEntity.ok(new PageTicketLogDto(ticketLogs.getContent(),
             page, pageSize, ticketLogs.getTotalPages(), ticketLogs.getTotalElements()));
